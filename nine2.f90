program densidad
        ...
        integer, parameter :: np=10000, pbin=1000, bines=10
        integer :: i,j,k, bin
        real ,dimension(np) :: x, y, xo, yo, ro
        integer, dimension(np) :: indx
        integer, dimension(bines) :: h
        real(kind=k1), dimension(np) :: r
        real :: M0, r1, r2, den, abin

        open(10,file='ocho.dat',status='old')
        do i=1,np
                read(10,*) x(i), y(i)
 ...
 !       close(11)
   !*************************************************************     
        open(12,file='Galaxy_binporNP.dat',status='unknown')
        M0 = 1. !e12/10000.
        r1 = 0
        do i=1,bines
                j   = i*pbin
